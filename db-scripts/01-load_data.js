...
db.users.insert({ username: "big_boss",     name: "John",       surname: "???" })
...
db.users.find().forEach((insertedUser) => {
    for (let walletNum = 0; walletNum <= NUM_WALLETS; walletNum++) {
        db.wallets.insert({
            "userId":   insertedUser._id.str,
            "alias":    `${insertedUser.username}_wallet_${walletNum}`,
